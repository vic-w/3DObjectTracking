...
int main() {
  ...
  auto camera_l_ptr{std::make_shared<rbgt::DatasetRBOTCamera>()};
  camera_l_ptr->Init("camera_left", dataset_path, "ape_stereo", "image_l_", 0);

  auto camera_r_ptr{std::make_shared<rbgt::DatasetRBOTCamera>()};
  camera_r_ptr->Init("camera_right", dataset_path, "ape_stereo", "image_r_", 0);
